...
function createBuildManager(opt) {
  ...
  const client = opt.serverRoot && createClientStream()
  ...
  const primaryJob = {
    fn: createESBuilder(opt.esbuildOptions, Boolean(opt.serverRoot), initPlugin({fileSet: watcher})),
...
function createESBuilder(esbuildOptions, shouldInject, plugin) {
  let esb = null
  return async () => {
    const ts = Date.now()
    try {
      if (esb === null) {
        esb = await esbuild.build({
          ...
          inject: shouldInject
            ? [...(esbuildOptions.inject ?? []), INJECT_MODULE]
...
function outputBuildStatus(operation, ts) {
  ...
  process.stdout.write(`[rum] ${createdAt} >> ${operation} ${elapsed}s\n`)
}
