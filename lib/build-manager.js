...
function createBuildManager(opt) {
  ...
  const browserify = fromCLI(opt.browserifyParams, {cache: {}, packageCache: {}})
    // Inject the client-side listener for "reload" events
    // Unlike the CLI --entry option, this should not cause transforms to run on node_modules
    .add(require.resolve('./client'))
